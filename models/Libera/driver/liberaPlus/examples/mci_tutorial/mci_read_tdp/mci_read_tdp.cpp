 ...
 * All Rights Reserved.
 *
 * $Id: mci_read_tdp.cpp 22034 2015-06-19 08:32:00Z damijan.skvarc $
 */
...
int main(int a_argc, char* a_argv[])
{
    ...
    /* get registry node referring to turn-by-turn (TDP) data on demand signal */
    mci::Node node = root.GetNode(mci::Tokenize(cTdpDod));

    /* instantiate remote signal entity */
    isig::SignalSourceSharedPtr signal = mci::CreateRemoteSignal(node);

    /* tdp_synthetic signal is "data on demand" signal whose basic data type is signed 32 bit */
    ...
    /* open client for reading 10 signal atoms regardless the trigger event */
    /* to read on trigger event use eModeDodOnEvent                         */
    auto ret = client.Open(isig::eModeDodNow, 10);
