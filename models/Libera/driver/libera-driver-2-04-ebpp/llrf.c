/* $Id: llrf.c 2192 2008-10-07 09:13:06Z matejk $ */

/** \file llrf.c */
/** Implements Libera Low Level RF (LLRF) specifics. */
...
/** All LLRF ARM triggers (MC module) */
const int LLRF_Arms  = TRIGGER_BIT(T_TRIG_POST_MORTEM) |
                       TRIGGER_BIT(T_TRIG_FA1);
/** All LLRF TRIGGER triggers (MC module) */
const int LLRF_Trigs = TRIGGER_BIT(T_TRIG_TRIGGER) |
 ...
 */
static ssize_t
libera_dd_write_CBfifo_LLRF(size_t cb_PostFilt,
                            size_t cb_StartAddr, 
                            size_t cb_AddrStep,
                            size_t cb_SnglSize,
                            size_t cb_NoTrans)
...
/** Initialize Libera LLRF specific internal device structures.
 *
 * Initializes Libera LLRF specific internal device structures. 
 */
int libera_init_specific(void)
{
    ...
    /* Valid triggers */
    event->ValidTrigVector = LLRF_Arms;
...
/** Libera Configuration Device: LLRF specific GET methods.
 *
 * Takes care of CFG GET actions on LLRF Libera family member specific
 * parameters.
 */
int 
...
/** Libera Configuration Device: LLRF specific SET methods.
 *
 * Takes care of CFG SET actions on LLRF Libera family member specific
 * parameters.
 */
int 
...
libera_valid_trigger(struct libera_event_device *event, 
                     int trigno)
{
    /* NOTE: For proper LLRF operation, only one TRIGGER trigger
     *       is allowed after the ARM trigger and vice versa.
     */
    if ( event->ValidTrigVector & (TRIGGER_BIT(trigno)) ) {
        if (event->ValidTrigVector & LLRF_Arms)
            event->ValidTrigVector = LLRF_Trigs;
        else if (event->ValidTrigVector & LLRF_Trigs)
            event->ValidTrigVector = LLRF_Arms;
        return 1;
    } else
        return 0;
...
/** Libera LLRF specific SC trigger handler */
void 
		       ...
		       unsigned long m, unsigned int i)
{
    switch(i) 
    {
    case T_TRIG_FA0: /* LLRF_TRIGGER */
	/* NOTE: T_TRIG_FA0 (LLRF_TRIGGER) should be treated the same as
	 *       TRIGGER trigger in LLRF case.
	...
	PDEBUG2("SC LLRF_TRIGGER: LST = 0x%08lx%08lx, j = %lu\n",
                ULL(*stamp), jiffies);
...
/** Libera LLRF specific MC trigger handler */
void 
		       ...
		       unsigned long m, unsigned int i)
{
	 ...
	 *       queue.
	 */
	PDEBUG2("MC LLRF_TRIGGER: LMT = 0x%08lx%08lx, j = %lu\n",
                ULL(*stamp), jiffies);
    ...
    case T_TRIG_FA1: /* LLRF_ARM */
	PDEBUG2("MC LLRF_ARM: LMT = 0x%08lx%08lx, j = %lu\n",
                ULL(*stamp), jiffies);
        libera_renew_HB_offset(event, stamp);
	/* Notify userland about new event  */
	libera_send_event(LIBERA_EVENT_FA,
...
/** Libera LLRF DD Device: Called on lseek()
 *
...
libera_dd_llseek_specific(struct file *file, loff_t time, int whence)
{
    ...
    case 0: /* SEEK_SET */
    case 2: /* SEEK_END */
	return -EINVAL; // Not supported in LLRF
        break;
				    ...
				    libera_hw_time_t *slmt)
{
			    ...
			    else
    {
	    ...
	    PDEBUG2("Copied %u LLRF samples to Userland.\n", samples_written);
	}
	else
	{
	    /* DMA FIFO empty ?!
...
/** Libera LLRF DD Device: Called on read()
 *
			...
			size_t count, loff_t *f_pos)
{
    ...
    if ( count > 
         (LLRF_MAX_SAMPLES*sizeof(libera_sample_t)) ) {
    ...
    sizeSD = sizeS; //No decimation in LLRF
    ...
    CB_ret = libera_dd_write_CBfifo_LLRF(0,
                                         atom_start,
                                         1,
                                         1,
                                         sizeA);
