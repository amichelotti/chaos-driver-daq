...
typedef enum {
    ...
    /* Added to be able to compile driver for Bunch by Bunch */
...
#define LIBERA_HB_SIZE		(256*1024*1024)
#define LIBERA_DD_SAMP_SIZE	(2) /* Must be equal to sizeof(libera_sample_t) */
#define LIBERA_DD_ATOM_SIZE	(8) /* size of atom in bytes (64-bit) */
#define LIBERA_DD_CIRCBUF_SAMP  (LIBERA_HB_SIZE / LIBERA_DD_SAMP_SIZE) //<! No. of samples in HB.
#define LIBERA_DD_CIRCBUF_ATOMS (LIBERA_HB_SIZE / LIBERA_DD_ATOM_SIZE) //<! No. of atoms in HB.
 ...
 *
 * This is the atom, used for reading from the FPGA interface. A conversion
 * to logical atom size is done when sending into the userspace.
 *
 */
typedef struct {
...
/**
 * Number of samples in the logical atom size as used in the userspace.
 */
extern unsigned long libera_logical_atom_len;
